...
BOOST_AUTO_TEST_CASE(packed_vector_iterator_test)
{
    ...
    auto reverse_range = boost::adaptors::reverse(boost::make_iterator_range(packed_vec.cbegin(), packed_vec.cend()));
    ...
    auto mut_reverse_range = boost::adaptors::reverse(boost::make_iterator_range(packed_vec.begin(), packed_vec.end()));
    BOOST_CHECK_EQUAL(reverse_range.size(), packed_vec.size());
    for (auto idx : util::irange<std::size_t>(0, packed_vec.size()))
    {
